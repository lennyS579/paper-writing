...
这是一个面向中文母语科研作者的跨智能体科研写作总控 Skill。它根据研究阶段、已有材料和风险状态，安排下一步任务，并把研究问题、证据、文献、写作、翻译、预审和返修接成一条可追溯的流程。
...
| 中文定位 | 证据优先的科学论文写作工作流 |
| 适用环境 | 支持读取 Markdown/SKILL.md 指令的 AI 助手或智能体框架 |
...
本仓库的核心内容是 Markdown 工作流和参考模板，不依赖某一个 AI 平台。只要目标助手能够读取 `SKILL.md` 和 `references/` 文件，就可以手动接入或转换为对应平台的提示词/技能格式。

仓库提供了 [install.sh](install.sh)，可以把 Skill 安装到常见的用户级技能目录。安装脚本默认不覆盖已有目录；如果目标已存在，会停止并提示你先处理旧版本。
...
也可以先克隆仓库，再运行 `./install.sh --codex`。复制后重启 Codex 或刷新 Skills 列表。`agents/openai.yaml` 是 Codex 界面元数据，不影响 Skill 在其他平台使用。

### 其他 AI 助手或智能体框架

将 `evidence-first-paper-writing/SKILL.md` 作为系统指令或项目 Skill 加载，并在需要时读取 `references/` 下的模板、路由表和检查清单。若目标平台有自己的 Skill 元数据格式，只需转换 frontmatter 和安装路径，不需要改变核心流程。

通用目标目录可以直接使用：

```bash
curl -fsSL https://raw.githubusercontent.com/lennyS579/paper-writing/master/install.sh | sh -s -- --target "$HOME/.agents/skills"
```

也可以下载仓库 ZIP，解压后将 `evidence-first-paper-writing/` 文件夹放入目标平台的 Skills 目录。

### Claude Code

```bash
curl -fsSL https://raw.githubusercontent.com/lennyS579/paper-writing/master/install.sh | sh -s -- --claude
```

### WorkBuddy

WorkBuddy 支持在技能管理界面上传本地技能包，也支持从 GitHub 导入。命令行用户可以使用：

```bash
curl -fsSL https://raw.githubusercontent.com/lennyS579/paper-writing/master/install.sh | sh -s -- --workbuddy
```

安装后重启 WorkBuddy，或在技能管理界面启用。项目级使用时，也可以安装到：

```bash
curl -fsSL https://raw.githubusercontent.com/lennyS579/paper-writing/master/install.sh | sh -s -- --target "$PWD/.agents/skills"
```

官方 WorkBuddy 文档说明了本地技能包上传方式及 `~/.workbuddy/skills/`、`.agents/skills/` 等发现路径，具体以当前客户端版本为准。

只加载总控 Skill 时，研究问题收敛、证据边界、提纲和停止条件仍然可用。
